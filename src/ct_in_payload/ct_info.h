...
struct dnat_info {
	...
	unsigned int skb_hash; // this hash just for balance 
};
...
void dump_dnat_info(struct dnat_info * di) {
	...
	printk("di->skb_hash=%u, sizeof(struct dnat_info) =%lu\n", di->skb_hash, sizeof(struct dnat_info));
	...
	printf("di->skb_hash=%u, sizeof(struct dnat_info) =%lu\n", di->skb_hash, sizeof(struct dnat_info));
	#endif
}
#endif
